# Unit 01: HTML & CSS Basics

## Overview
By the end of the unit, students will be able to create and style simple web pages using semantic HTML and basic CSS.


## How to Use This Unit  

- **Always** do your best work
- Use the background info and examples provided
- Apply what you've learned to your projects
- Debug your work **as you complete each part of your project** 

---

## Learning Objectives
By the end of this unit, students will:
- Understand the structure of an HTML document
- Use semantic HTML elements to create accessible web pages
- Apply basic CSS to style text, colors, and layouts
- Build a simple multi-page website

---

## Resources

* [w3schools](https://www.w3schools.com/)
* [Bro Code: HTML & CSS](https://youtu.be/HGTJBPNC-Gw?si=GHParQwTeaQ6TeXV)
* [Bro Code: JavaScript](https://youtube.com/playlist?list=PLZPZq0r_RZOO1zkgO4bIdfuLpizCeHYKv&si=9AhKvC9ajwZiK9ej)
* [Mosh: JavaScript](https://youtube.com/playlist?list=PLTjRvDozrdlxEIuOBZkMAK5uiqp8rHUax&si=nH6K_6K6jOUC3CnL)
* [Traversy Media: JavaScript for Beginners](https://youtu.be/hdI2bqOjy3c?si=q99UMBD1XGxNZmJx)

### Online Validators

* [W3C HTML Online Validator](https://validator.w3.org/#validate_by_upload)  
* [W3C CSS Validator (Jigsaw)](https://jigsaw.w3.org/css-validator/#validate_by_upload)

---

## Projects

<!--
- [Project 00](url)

- [Project 03](url)
- [Project 02](url)
-->
- [Project 01](url)


---


